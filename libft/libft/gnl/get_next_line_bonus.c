...
char	*ft_free2(char *collector, char *buf, int flag_both)
{
	if (flag_both == 0)
	{
		free(collector);
...
char	*ft_fill_coll(int fd, char *collector, int *end_flag, size_t *line_len)
{
	...
	if (!buf)
		return (ft_free2(collector, NULL, 0));
	while (*end_flag > 0 && !(ft_strchr_mod(collector, '\n', line_len)))
	{
		*end_flag = read(fd, buf, BUFFER_SIZE);
		if (*end_flag < 0)
			return (ft_free2(collector, buf, 1));
		buf[*end_flag] = '\0';
		collector = ft_strjoin_mod2(collector, buf);
		if (!collector)
			return (NULL);
	}
	free(buf);
